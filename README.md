# Beta Build
Dorian Englisbee
Tyler Hayward
Richard Xiong

## Licenses
- ship, animations for ship, energy waves, teleport, dash explosion
    - https://pixel-carvel.itch.io/player-spaceship

- ship weapons, and abilities (shields, rockets)
    - https://foozlecc.itch.io/void-main-ship

- enemy ships, animations for movement, attacks, and explosions
    - https://pixel-carvel.itch.io/shoot-em-up-enemies-ships-t1

- backgounds, asteroids
    - https://foozlecc.itch.io/void-environment-pack
    - https://creativecommons.org/publicdomain/zero/1.0/ (license)

- backgrounds for level 3
    - https://pixelfranek.itch.io/parallax-city-looping

- coins
    - https://dani-maccari.itch.io/platformer-metroidvania-pixel-items-free-assets

- UI
    - https://crusenho.itch.io/complete-ui-essential-pack

- UI and Fonts
    - https://free-game-assets.itch.io/free-gui-for-cyberpunk-pixel-art

- Portal Art
    - https://elthen.itch.io/2d-pixel-art-portal-sprites
    - [License](https://www.patreon.com/posts/27430241)

- Fairy Sprite
    - https://papoycore.itch.io/fairy

- Tileset
    - https://aske4.itch.io/free-space-station

- Heart Animation
    - https://nicolemariet.itch.io/pixel-heart-animation-32x32-16x16-freebie

- ships
    - https://disruptorart.itch.io/tiny-ships-free-spaceships


## Additions
A list of changes since the beta
- Main Menu Background Music
- Added Custom Mouse Cursor
- Main Menu Backround Images
- Main Menu Button Click Sound
- Added sound to weapons 
- Added low health sound effect
